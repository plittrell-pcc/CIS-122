...
# Lab:    CIS-122 Lab6
# Date:   July 15, 2026
# Desc:   Added input validation to prevent invalid user input from crashing
...
def main():
    """
    Main function to run the Personal Fitness Tracker program.
    """
    continue_program = 'y'
...
def get_exercise_type():
    """
    Get exercise type from the user and validate the range
    ...
    exercise_type = v.get_integer("Enter the type of exercise (1, 2, 3, 4, 5): ")

    # Validate input choice is within range
    while exercise_type not in [1, 2, 3, 4, 5]:
        print("Invalid choice. Please select a number between 1 and 5.")
        exercise_type = v.get_integer("Enter the type of exercise (1, 2, 3, 4, 5): ")

    result_type = exercise_type
    return result_type
...
def get_custom_name():
    ...
    """
    name = v.get_string("Enter the name of your custom exercise: ")
...
def get_custom_rate():
    """
    Get the calorie burn rate for the unlisted exercise and ensure it is positive
    :return: float, custom calorie burn rate
    """
    rate_input = v.get_real("Enter the calories burned per minute for this exercise: ")

    # Check for positive number
    while rate_input <= 0:
        print("Invalid rate. Calorie burn rate must be greater than 0.")
        rate_input = v.get_real("Enter the calories burned per minute for this exercise: ")

    result_rate = rate_input
    return result_rate


def get_workout_minutes():
    """
    Get workout minutes from the user and ensure they are positive
    :return: float, workout minutes
    """
    workout_minutes = v.get_real("Enter the number of minutes you exercised: ")

    # Check for positive number
    while workout_minutes <= 0:
        print("Invalid duration. Workout minutes must be greater than 0.")
        workout_minutes = v.get_real("Enter the number of minutes you exercised: ")

    result_minutes = workout_minutes
...
def calc_calories(minutes, rate_per_minute):
    ...
    # Variable assignment, avoiding returning an equation directly
...
def get_keep_going():
    ...
    :return: str, user choice ('y' or 'n')
    """
    choice = v.get_y_or_n("Would you like to calculate another exercise? (y/n): ")
